...
class BasicBlock_ss(nn.Module):
    ''' basic residual block, for subsample
    seems not exactly the same as resnet, need further investigation '''
...
class BasicBlock_us(nn.Module):
    ''' basic residual block, for upsample
    seems not exactly the same as resnet, need further investigation '''
...
class FresUNet_HM(nn.Module):
    """FresUNet segmentation network for hoekman decomposition"""
    ...
    def __init__(self, input_nbr, label_nbr):
        ...
        # batchNorm
        self.bn = nn.BatchNorm2d(num_features=input_nbr)
