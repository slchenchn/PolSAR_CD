...
''' 1. 使得输入数据的通道数可以自定义 
    ...
    3. 将纯复数改为混合网络，前面几个block是复数，后面改成实数
'''
...
class BasicBlock(nn.Module):
    '''real-valued basic residual block 
        adapt from pytorch official implementation    
    '''
    expansion: int = 1
...
class complex_BasicBlock2(nn.Module):
    ''' adapt from deeplabv3 '''
    expansion = 1
    ...
    def __init__(self, in_channels, channels, stride=1, dilation=1):
        ...
        self.conv1 = ComplexConv2d(in_channels, channels, kernel_size=3, stride=stride, padding=dilation, dilation=dilation, bias=False)
        self.bn1 = NaiveComplexBatchNorm2d(channels)

        self.conv2 = ComplexConv2d(channels, channels, kernel_size=3, stride=1, padding=dilation, dilation=dilation, bias=False)
        self.bn2 = NaiveComplexBatchNorm2d(channels)

        if (stride != 1) or (in_channels != out_channels):
            conv = ComplexConv2d(in_channels, out_channels, kernel_size=1, stride=stride, bias=False)
            bn = NaiveComplexBatchNorm2d(out_channels)
            self.downsample = nn.Sequential(conv, bn)
        else:
            self.downsample = nn.Sequential()
    ...
    def forward(self, x):
        ...
        out = (self.bn1(self.conv1(x))) # (shape: (batch_size, channels, h, w) if stride == 1, (batch_size, channels, h/2, w/2) if stride == 2)
        out = self.bn2(self.conv2(out)) # (shape: (batch_size, channels, h, w) if stride == 1, (batch_size, channels, h/2, w/2) if stride == 2)

        out = out + self.downsample(x) # (shape: (batch_size, channels, h, w) if stride == 1, (batch_size, channels, h/2, w/2) if stride == 2)

        out = complex_relu(out) # (shape: (batch_size, channels, h, w) if stride == 1, (batch_size, channels, h/2, w/2) if stride == 2)
...
class complex_ResNet(nn.Module):
    ...
    def __init__(
        ...
        complex_norm_layer: Optional[Callable[..., nn.Module]] = None
    ) -> None:
        super().__init__()
        if complex_norm_layer is None:
            real_norm_layer = nn.BatchNorm2d
            complex_norm_layer = NaiveComplexBatchNorm2d
        self._complex_norm_layer = complex_norm_layer
        self._real_norm_layer = real_norm_layer

        self.inplanes = int(64*channel_scale)
        self.dilation = 1
        ...
        self.conv1 = ComplexConv2d(input_nbr, int(self.inplanes/2), kernel_size=3, stride=2, padding=1,
                               bias=False)
        self.bn1 = complex_norm_layer(int(self.inplanes/2))
        self.relu = ComplexReLU()
        self.conv2 = ComplexConv2d(int(self.inplanes/2), int(self.inplanes), kernel_size=3, stride=2, padding=1,
                               bias=False)
        self.bn2 = complex_norm_layer(int(self.inplanes))
        self.relu2 = ComplexReLU()


        self.layer1 = self._make_layer(BasicBlock, int(64*channel_scale), layers[0])
        self

        self.layer2 = self._make_layer(BasicBlock, int(128*channel_scale), layers[1], stride=2,
                                       dilate=replace_stride_with_dilation[0])
        self.layer3 = self._make_layer(BasicBlock, int(256*channel_scale), layers[2], stride=2,
                                       dilate=replace_stride_with_dilation[1])
        self.layer4 = self._make_layer(BasicBlock, int(512*channel_scale), layers[3], stride=2,
                                       dilate=replace_stride_with_dilation[2])
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        self.fc = nn.Linear(int(512 * channel_scale * block.expansion), num_classes)

        # cancel complex initialization
    ...
    def _make_layer(self, block: Type[Union[BasicBlock, Bottleneck]], planes: int, blocks: int,
        ...
        norm_layer = self._real_norm_layer
        downsample = None
    ...
    def _make_complex_layer(self, block: Type[Union[complex_BasicBlock, complex_Bottleneck]], planes: int, blocks: int,
                    stride: int = 1, dilate: bool = False) -> nn.Sequential:
        norm_layer = self._complex_norm_layer
        downsample = None
        previous_dilation = self.dilation
        if dilate:
            self.dilation *= stride
...
class surreal_ResNet(nn.Module):
    ...
    def __init__(
        ...
        self.conv1 = ComplexConv2Deffgroup(input_nbr, int(self.inplanes/2), kern_size=(3, 3), stride=(2, 2), padding=(1, 1))
        self.relu1 = manifoldReLUv2angle(int(self.inplanes/2))
        self.conv2 = ComplexConv2Deffgroup(int(self.inplanes/2), self.inplanes, kern_size=(3, 3), stride=(2, 2), padding=(1, 1))
        self.relu2 = manifoldReLUv2angle(self.inplanes)
        self.dist = ComplexLinear_per_channel(self.inplanes, 128)
...
def _complex_resnet(
    ...
    **kwargs: Any
    ...
    model = complex_ResNet(block, layers, channel_scale=channel_scale, **kwargs)
    if pretrained:
        state_dict = load_state_dict_from_url(model_urls[arch],
                                              progress=progress)
        model.load_state_dict(state_dict)
...
def _surreal_resnet(
    ...
    **kwargs: Any
    ...
    model = surreal_ResNet(block, layers, channel_scale=channel_scale, **kwargs)
    if pretrained:
        state_dict = load_state_dict_from_url(model_urls[arch],
                                              progress=progress)
        model.load_state_dict(state_dict)
...
class complex_ResNet_BasicBlock_OS8(nn.Module):
    def __init__(self, num_layers, pretrained=False, input_nbr=4, channel_scale=0.25):
        ...
        self.layer4 = make_layer(BasicBlock2, in_channels=int(channel_scale*128), channels=int(256*channel_scale), num_blocks=num_blocks_layer_4, stride=1, dilation=2)
        self.layer5 = make_layer(BasicBlock2, in_channels=int(channel_scale*256), channels=int(512*channel_scale), num_blocks=num_blocks_layer_5, stride=1, dilation=4)
...
class surreal_ResNet_BasicBlock_OS8(nn.Module):
    def __init__(self, num_layers, pretrained=False, input_nbr=4, channel_scale=0.25):
        super().__init__()

        if num_layers == 18:
            resnet = surreal_resnet18(input_nbr=input_nbr, channel_scale=channel_scale)
            # load pretrained model:
            if pretrained:
                resnet.load_state_dict(torch.load("/root/deeplabv3/pretrained_models/resnet/resnet18-5c106cde.pth"))
                print ("pretrained resnet, 18")
...
def complex_resnet18(pretrained: bool = False, progress: bool = True, channel_scale=0.25, **kwargs: Any) -> complex_ResNet:
    r"""ResNet-18 model from
    `"Deep Residual Learning for Image Recognition" <https://arxiv.org/pdf/1512.03385.pdf>`_
    ...
    Args:
        ...
        progress (bool): If True, displays a progress bar of the download to stderr
    """
    return _complex_resnet('resnet18', complex_BasicBlock, [2, 2, 2, 2], pretrained, progress, channel_scale=channel_scale,
                   **kwargs)


def surreal_resnet18(pretrained: bool = False, progress: bool = True, channel_scale=0.25, **kwargs: Any) -> complex_ResNet:
    r"""ResNet-18 model from
    `"Deep Residual Learning for Image Recognition" <https://arxiv.org/pdf/1512.03385.pdf>`_
    ...
    Args:
        ...
        progress (bool): If True, displays a progress bar of the download to stderr
    """
    return _surreal_resnet('resnet18', complex_BasicBlock, [2, 2, 2, 2], pretrained, progress, channel_scale=channel_scale,
                   **kwargs)
